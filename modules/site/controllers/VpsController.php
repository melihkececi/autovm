...
class VpsController extends Controller
{
	...
	public function actionSslstartVipHosting(){
		VipHosting::SslSettings();
	}
	...
	public function actionPasswordSend(){
		...
		if($vps){
			...
			if($userEmail){
				if(Yii::$app->VipHostingmailer->send($userEmail->email, "{$vps->os->name} VDS Şifresi", "Şifreniz : ".$vps->password." <br><br> Lütfen şifrenizi kimse ile paylaşmayınız.")){
	...
	public function actionRdnsPendingEdit(){
		$rdnsModuleVipHosting = VipHosting::SiteVpsRdnsPendingEdit();
		if($rdnsModuleVipHosting['ok'] == true){
			return $this->renderAjax('rdns-edit-pending', $rdnsModuleVipHosting['response']);
		}
		else{
	...
	public function actionRdnsPendingDelete(){
		$rdnsModuleVipHosting = VipHosting::SiteVpsRdnsPendingDelete();
		if($rdnsModuleVipHosting['ok'] == true){
			return $this->renderAjax('rdns-pending-delete-success', $rdnsModuleVipHosting['response']);
		}
		else{
			return $rdnsModuleVipHosting['response'];
		}
	...
	public function actionChangeRdnsPending(){
		return VipHosting::PostChangeRdnsPending();
	}
	
    public function actionChangeRdns()
    {
		return VipHosting::PostChangeRdns();
    }
	
    public function actionRdnsDelete()
    {
		$rdnsModuleVipHosting = VipHosting::SiteVpsRdnsDelete();
		if($rdnsModuleVipHosting['ok'] == true){
			return $this->renderAjax('rdns-delete', $rdnsModuleVipHosting['response']);
		}
		else{
			return $rdnsModuleVipHosting['response'];
		}
    ...
    {
		$rdnsModuleVipHosting = VipHosting::SiteVpsRdnsEdit();
		if($rdnsModuleVipHosting['ok'] == true){
			return $this->renderAjax('rdns-edit', $rdnsModuleVipHosting['response']);
		}
		else{
			return $rdnsModuleVipHosting['response'];
		}
	...
	public function actionCreateRdns(){
		return VipHosting::PostCreateRdns();
	}
	
	public function actionRdnsSelectChangeCreate(){
		$rdnsModuleVipHosting = VipHosting::SiteVpsRdnsSelectChangeCreate();
		if($rdnsModuleVipHosting['ok'] == true){
			return $this->renderAjax('rdns-select-change-create', $rdnsModuleVipHosting['response']);
		}
		else{
			return $rdnsModuleVipHosting['response'];
		}
	...
	public function actionRdnsCreate(){
		$rdnsModuleVipHosting = VipHosting::SiteVpsRdnsCreate();
		if($rdnsModuleVipHosting['ok'] == true){
			return $this->renderAjax('rdns-create', $rdnsModuleVipHosting['response']);
		}
		else{
			return $rdnsModuleVipHosting['response'];
		}
	...
	public function actionRdnsResultChange($id){
		$rdnsModuleVipHosting = VipHosting::SiteVpsRdnsResultChange($id);
		if($rdnsModuleVipHosting != false){
			return $this->renderAjax('rdns-result-change', $rdnsModuleVipHosting);
		}
	}
    ...
    {
		$VipHostingStepInstall = VipHosting::StepInstall($id);
		return $VipHostingStepInstall;
    }

    public function actionInstall()
    {
		Yii::$app->response->format = Response::FORMAT_JSON;
		$VipHostingVpsID = Yii::$app->request->post('vpsId');
		$VipHostingVps = Vps::find()->where(['user_id' => Yii::$app->user->id, 'id' => $VipHostingVpsID])->active()->one();
		$OsLists = json_decode($VipHostingVps->plan->os_lists);
		if(is_array($OsLists)){
			$VipHostingOsID = Yii::$app->request->post('osId');
			if(!in_array($VipHostingOsID, $OsLists)){
		...
		if(isset($errorFound)){
			...
			$VipHostingOsInstall = VipHosting::InstallOs();
			return $VipHostingOsInstall;
		}
    }
