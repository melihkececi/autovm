<?php

namespace app\models;

use Yii;
use yii\base\Model;
use yii\helpers\Url;
use yii\web\Response;
use yii\web\Controller;
use yii\data\Pagination;
use yii\web\NotFoundHttpException;


class VipHostingSettings extends Model{
	
	# VipHosting_default 
	# sabitini sadece şu işletim sistemlerinde kullanın.
	# Centos 7
	# Centos 6.8
	# Centos 8
	# Debian 8.5
	# Debian 9.6
	# Debian 9.9
	# Ubuntu 16.04
	# Ubuntu 18.04
	# Windows 2003
	# Windows 2008
	# Windows 2012
	# Windows 2019
	# Windows 10
	
	
	# Konsola SSL Eklemek için Aşağıya Sertifika ve Private Key kodlarını giriniz. 
	# Ardından autovm nin kurulu olduğu adresinizin sonuna aşağıdaki satırı ekleyin.
	# /site/vps/sslstartVipHosting
	# Örnek : https://panel.VipHosting.com.tr/site/vps/sslstartVipHosting
	
	public static  $ssl_crt = "-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----";
	
	public static  $ssl_key = "-----BEGIN PRIVATE KEY-----

p5+8Rtf5Cvlupmylj3hY+FxV
-----END PRIVATE KEY-----";
	
	
	public static function autoPassword(){
		
        $one = 'abcdefghijklmnopqrstuvwxyz';
        $two = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
        $three = '1234567890';

        $password = '';

        for ($i=0; $i<3; $i++) {
            $password .= $one[mt_rand(0, strlen($one)-1)];
        }

        for ($i=0; $i<3; $i++) {
            $password .= $two[mt_rand(0, strlen($two)-1)];
        }

        for ($i=0; $i<3; $i++) {
            $password .= $three[mt_rand(0, strlen($three)-1)];
        }
		
		return $password;
		
	}
	
	
	public static function networkSettings($variable, $ipDetail = false){
		
		$variables = [
			'centos7' => 'VipHosting_default',
			'centos6_8' => 'VipHosting_default',
			'centos8' => 'VipHosting_default',
			'debian8_5' => 'VipHosting_default',
			'debian9_6' => 'VipHosting_default',
			'debian9_9' => 'VipHosting_default',
			'ubuntu16_04' => 'VipHosting_default',
			'ubuntu18_04' => 'VipHosting_default',
			'ubuntu19_04' => 'VipHosting_default',
			'windows2003' => 'VipHosting_default',
			'windows2008' => 'VipHosting_default',
			'windows7' => 'VipHosting_default',
			'windows8' => 'VipHosting_default',
			'windows2012' => 'VipHosting_default',
			'windows2016' => 'VipHosting_default',
			'windows2019' => 'VipHosting_default',
			'windows10' => 'VipHosting_default'
		];
		
		return $variables[$variable];
	}
	
	public static function isletimSistemleri(){
		# VipHosting_default = VipHostingun kendi ayarları
		/*
			'örnek' => 
						'os_name' => Veritabanında os isimi
						'networkSettings' => Network Ayarlaması için gereken kodlar. Array içinde yazılması gerekir. Örnek = array($networkAyar1, $networkAyar2)
						'windows' => Eklediğinz işletim sistemi windows mu ? windows ise true değil ise false
		*/
		
		// Array içindeki VipHostingun eklediği sabitler değişemez. Değiştirildiğinde default kodlar okunmaz.
		
		return [
			'centos7' => ['os_name' => 'centos 7', 'networkSettings' => VipHostingSettings::networkSettings('centos7'), 'windows' => false],
			'centos6_8' => ['os_name' => 'centos 6.8', 'networkSettings' => VipHostingSettings::networkSettings('centos6_8'), 'windows' => false],
			'centos8' => ['os_name' => 'centos 8', 'networkSettings' => VipHostingSettings::networkSettings('centos8'), 'windows' => false],
			'debian8_5' => ['os_name' => 'debian 8.5', 'networkSettings' => VipHostingSettings::networkSettings('debian8_5'), 'windows' => false],
			'debian9_6' => ['os_name' => 'debian 9.6', 'networkSettings' => VipHostingSettings::networkSettings('debian9_6'), 'windows' => false],
			'debian9_9' => ['os_name' => 'debian 9.9', 'networkSettings' => VipHostingSettings::networkSettings('debian9_9'), 'windows' => false],
			'ubuntu16_04' => ['os_name' => 'ubuntu 16.04', 'networkSettings' => VipHostingSettings::networkSettings('ubuntu16_04'), 'windows' => false],
			'ubuntu18_04' => ['os_name' => 'ubuntu 18.04', 'networkSettings' => VipHostingSettings::networkSettings('ubuntu18_04'), 'windows' => false],
			'ubuntu19_04' => ['os_name' => 'ubuntu 19.04', 'networkSettings' => VipHostingSettings::networkSettings('ubuntu19_04'), 'windows' => false],
			'windows2003' => ['os_name' => 'windows 2003', 'networkSettings' => VipHostingSettings::networkSettings('windows2003'), 'windows' => true],
			'windows2008' => ['os_name' => 'windows server 2008', 'networkSettings' => VipHostingSettings::networkSettings('windows2008'), 'windows' => true],
			'windows7' => ['os_name' => 'windows 7', 'networkSettings' => VipHostingSettings::networkSettings('windows2008'), 'windows' => true],
			'windows8' => ['os_name' => 'windows 8', 'networkSettings' => VipHostingSettings::networkSettings('windows2008'), 'windows' => true],
			'windows2012' => ['os_name' => 'windows server 2012', 'networkSettings' => VipHostingSettings::networkSettings('windows2012'), 'windows' => true],
			'windows2016' => ['os_name' => 'windows server 2016', 'networkSettings' => VipHostingSettings::networkSettings('windows2012'), 'windows' => true],
			'windows2019' => ['os_name' => 'windows server 2019', 'networkSettings' => VipHostingSettings::networkSettings('windows2019'), 'windows' => true],
			'windows10' => ['os_name' => 'windows 10', 'networkSettings' => VipHostingSettings::networkSettings('windows10'), 'windows' => true]
			
			
		];
	}
	
}
