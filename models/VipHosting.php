...
use app\models\User;
use app\models\VipHostingSettings;
use app\models\Os;
use app\models\Vps;
use app\models\Log;
use app\models\Iso;
...
class VipHosting extends Model{
	
	
	public static function SslSettings(){
				
		$lisansKontrol = VipHosting::lisansKontrol();
		
		if($lisansKontrol == true){
		
			$SslDizin = dirname(dirname(__FILE__)).'/web/console/utils/self.pem';
			$SslCrt = VipHostingSettings::$ssl_crt;
			$SslKey = VipHostingSettings::$ssl_key;
	...
	public static function InstallOs(){
		...
		$Os_Find_VipHosting = VipHosting::Os_Find($vps->os->name, $ipAttributes);
		
		if($Os_Find_VipHosting != "licence0"):
		
			if(isset($Os_Find_VipHosting['network'])){
				
				$networkSettings = $Os_Find_VipHosting['network'];
				
				$extendContent = "def extend(username, password):\n\tvariables = {'vpsId': {$vps->id}, 'username' : username, 'password' : password, 'key' : \"".VipHostingSettings::$api_key."\"}\n\tr = requests.post(\"".Url::base(VipHostingSettings::$secure_url) . Yii::$app->urlManager->createUrl(['/api/vps/extend'])."\", data=variables)\n\tresponse = r.content\n\tjsonresponse = json.loads(response)\n\n\tif int(jsonresponse[\"status\"]) is 0:\n\t\treturn False\n\telif int(jsonresponse[\"status\"]) is 1:\n\t\treturn True\n\nif extend(os_username, password):\n\twrite('status', '5:100')\nelse:\n\twrite('status', '5:99')";
	...
	public static function Os_Variables($os_variable, $ipDetail = false){
		...
		$variables = [
			'centos7' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'centos6_8' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'centos8' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'debian8_5' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'debian9_6' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'debian9_9' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'ubuntu16_04' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'ubuntu19_04' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'ubuntu18_04' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'windows2003' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'windows2008' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'windows7' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'windows8' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'windows2012' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'windows2016' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'windows2019' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail)),
			'windows10' => array('network' => VipHosting::networkCommand($os_variable, $ipDetail))
		];
	...
	public static function Os_Find($osName, $ipDetail = false){
		...
		if($lisansKontrol == false){
			...
			$isletimSistemleri = VipHostingSettings::isletimSistemleri();
			
			foreach($isletimSistemleri as $key => $value){
				if(stripos($osName, $value['os_name']) !== FALSE){
					if($value['networkSettings'] == 'VipHosting_default'){
						$osSettings = VipHosting::Os_Variables($key, $ipDetail);
						if($osSettings == false){
							break;
						}
						else{
						...
						$networkSettings = VipHostingSettings::networkSettings($key, $ipDetail);
				...
				if($ip_id){
					...
					if($vps){
						...
						if($rdns){
							...
							$VipHosting_one_data_types = json_decode(Yii::$app->rdnsayar->__get('VipHosting_one_data_types'));
							
							if(is_array($VipHosting_one_data_types)){
								foreach($VipHosting_one_data_types as $VipHosting_one_data_type){
									if($type == $VipHosting_one_data_type){
										$rdnsdata = Rdnsdata::find()->where(['ana_ip' => $ip->ip])
										->andQuery(['type' => $VipHosting_one_data_type])
										->one();
										...
										->one();
										if($rdnsdata){
											$hataVar = "Daha önceden eklenmiş bir {$VipHosting_one_data_type} kaydı zaten var. Mevcut olanı güncelleyiniz";
										}
										if($rdns_status){
											$hataVar = "{$ip->ip} ip adresinin işlemde olan {$VipHosting_one_data_type} kaydı zaten var. Lütfen eklenmesini bekleyiniz.";
										}
									}
								}
							}
									...
									if(is_array($VipHosting_one_data_types)){
										foreach($VipHosting_one_data_types as $VipHosting_one_data_type){
											if($type == $VipHosting_one_data_type){
												...
												->andQuery(['!=', 'id', $id])
												->one();
												$rdns_status = Rdnsdurum::find()->where(['pending_type' => [1,2], 'ana_ip' => $rdnsIp, 'status' => 0, 'type' => $VipHosting_one_data_type])->one();
												if($rdnsdata){
													$hataVar = "Daha önceden eklenmiş bir {$VipHosting_one_data_type} kaydı zaten var. Mevcut olanı güncelleyiniz";
												}
												if($rdns_status){
													$hataVar = "{$rdnsIp} ip adresinin işlemde olan {$VipHosting_one_data_type} kaydı zaten var. Lütfen eklenmesini bekleyiniz.";
												}
											}
										}
									}
			...
			if($vps_ip){
				...
				if($vps){
					...
					if(is_array($VipHosting_one_data_types)){
						foreach($VipHosting_one_data_types as $VipHosting_one_data_type){
							if($type == $VipHosting_one_data_type){
								$rdnsdata = Rdnsdata::find()->where(['ana_ip' => $VpsIp, 'type' => $VipHosting_one_data_type])->one();
								$rdns_status = Rdnsdurum::find()->where(['ana_ip' => $VpsIp, 'pending_type' =>  [1,2], 'status' => 0, 'type' => $VipHosting_one_data_type])->one();
								if($rdnsdata){
									$hataVar = "Daha önceden eklenmiş bir {$VipHosting_one_data_type} kaydı zaten var. Mevcut olanı güncelleyiniz";
								}
								if($rdns_status){
									$hataVar = "{$VpsIp} ip adresinin işlemde olan {$VipHosting_one_data_type} kaydı zaten var. Lütfen eklenmesini bekleyiniz.";
								}
							}
						}
					}
	...
	public static function RdnsSettings(){
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            foreach ($model->getAttributes() as $key => $value) {
				...
				if($key == "VipHosting_one_data_types"){
	...
	public static function RdnsCronDataDownload(){
		...
		foreach($rdns as $rdnsValue){
			...
			foreach($serverIds as $serverId){
				...
				VipHosting::RdnsCronReadAndSave($result, $rdnsValue->id, $serverId);
				...
				if(is_array($sayfalar) && isset($sayfalar[1]) && count($sayfalar[1]) > 0){
					...
					foreach($sayfalar[1] as $sayfaUrl){
						...
						VipHosting::RdnsCronReadAndSave($resultPage, $rdnsValue->id, $serverId);
	...
	public static function RdnsCronDeleteOneType(){
		$RdnsayarDelete = Rdnsayar::find()->where(['key' => 'VipHosting_one_data_delete'])->one();
		if($RdnsayarDelete){
			if($RdnsayarDelete->value == 1){
				
				$RdnsayarTypes = Rdnsayar::find()->where(['key' => 'VipHosting_one_data_types'])->one();
	...
	public static function RdnsCronReadAndSave($read, $rdnsID, $rdnsServerID){
		...
		foreach($veri2[0] as $veriler){
			...
			if(!$rdnsFind){
				...
				$model->ttl = $ttl;
				$model->ana_ip = VipHosting::rdns_ip_coz($name);
				$model->save();
			}
