...
class CronController extends Controller
{
	...
	public function actionRdns(){
		$this->rdns_login();
		$this->rdns_data_download();
	}
	
	public function rdns_pend_request(){
		$this->rdns_pending_insert();
		$this->rdns_pending_update();
		$this->rdns_pending_delete();
	}
	
	public function delete_one_type(){
		VipHosting::RdnsCronDeleteOneType();
	}
	
	public function rdns_pending_insert(){
		VipHosting::RdnsCronInsert();
	}
	
	public function rdns_pending_update(){
		VipHosting::RdnsCronUpdate();
	}
	
	public function rdns_pending_delete(){
		VipHosting::RdnsCronDelete();
	}
	
	
	public function rdns_data_download(){
		VipHosting::RdnsCronDataDownload();
	}
	
	public function rdns_login(){
		VipHosting::RdnsCronlogin();
	}
        ...
        foreach ($servers as $server) {
            ...
            foreach ($process->batch(10) as $machines) {
                ...
                foreach ($machines as $machine) {
					...
					if(isset($machine->ip->ip) && !empty($machine->ip->ip)){
        ...
        foreach ($servers as $server) {
            ...
            foreach ($process->batch(10) as $machines) {
                ...
                foreach ($machines as $machine) {
					...
					if(!isset($machine->ip->ip) OR empty($machine->ip->ip)) continue;
